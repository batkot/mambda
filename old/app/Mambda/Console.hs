...
runConsoleGame :: (Has GameSettings m, MonadIO m) => Rendering.RenderingHandle -> m ()
runConsoleGame _ = do
    (height, (width, wrapMap)) <- (mapHeight &&& mapWidth &&& wrapMap) <$> get
    let walls = World.createWorldMap wrapMap height width
    food <- foodLocations height width
    sGlyph <- snakeGlyph <$> get
    void $ startGame walls (World.invisible south) (World.visible sGlyph (Vec2D (0,1))) food
  where
